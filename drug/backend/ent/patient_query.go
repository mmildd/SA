...
type PatientQuery struct {
	...
	// eager-loading edges.
	withPatientDrugAllergy *DrugAllergyQuery
...
// QueryPatientDrugAllergy chains the current query on the Patient_DrugAllergy edge.
func (pq *PatientQuery) QueryPatientDrugAllergy() *DrugAllergyQuery {
...
//  WithPatientDrugAllergy tells the query-builder to eager-loads the nodes that are connected to
// the "Patient_DrugAllergy" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PatientQuery) WithPatientDrugAllergy(opts ...func(*DrugAllergyQuery)) *PatientQuery {
	query := &DrugAllergyQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPatientDrugAllergy = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withPatientDrugAllergy; query != nil {
		...
		query.Where(predicate.DrugAllergy(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.PatientDrugAllergyColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.PatientDrugAllergy = append(node.Edges.PatientDrugAllergy, n)
		}
	}
