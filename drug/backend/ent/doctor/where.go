...
// HasDoctorDrugAllergy applies the HasEdge predicate on the "Doctor_DrugAllergy" edge.
func HasDoctorDrugAllergy() predicate.Doctor {
	return predicate.Doctor(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(DoctorDrugAllergyTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, DoctorDrugAllergyTable, DoctorDrugAllergyColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasDoctorDrugAllergyWith applies the HasEdge predicate on the "Doctor_DrugAllergy" edge with a given conditions (other predicates).
func HasDoctorDrugAllergyWith(preds ...predicate.DrugAllergy) predicate.Doctor {
	return predicate.Doctor(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(DoctorDrugAllergyInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, DoctorDrugAllergyTable, DoctorDrugAllergyColumn),
		)
