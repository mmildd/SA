...
// HasMedicineDrugAllergy applies the HasEdge predicate on the "Medicine_DrugAllergy" edge.
func HasMedicineDrugAllergy() predicate.Medicine {
	return predicate.Medicine(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(MedicineDrugAllergyTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, MedicineDrugAllergyTable, MedicineDrugAllergyColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasMedicineDrugAllergyWith applies the HasEdge predicate on the "Medicine_DrugAllergy" edge with a given conditions (other predicates).
func HasMedicineDrugAllergyWith(preds ...predicate.DrugAllergy) predicate.Medicine {
	return predicate.Medicine(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(MedicineDrugAllergyInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, MedicineDrugAllergyTable, MedicineDrugAllergyColumn),
		)
