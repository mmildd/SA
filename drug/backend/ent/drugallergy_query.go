...
func (daq *DrugAllergyQuery) QueryDoctor() *DoctorQuery {
	query := &DoctorQuery{config: daq.config}
...
func (daq *DrugAllergyQuery) QueryPatient() *PatientQuery {
	query := &PatientQuery{config: daq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := daq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (daq *DrugAllergyQuery) QueryManner() *MannerQuery {
	query := &MannerQuery{config: daq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := daq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (daq *DrugAllergyQuery) WithDoctor(opts ...func(*DoctorQuery)) *DrugAllergyQuery {
	query := &DoctorQuery{config: daq.config}
	for _, opt := range opts {
		opt(query)
	}
	daq.withDoctor = query
	return daq
}
...
func (daq *DrugAllergyQuery) WithPatient(opts ...func(*PatientQuery)) *DrugAllergyQuery {
	query := &PatientQuery{config: daq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//  WithManner tells the query-builder to eager-loads the nodes that are connected to
// the "manner" edge. The optional arguments used to configure the query builder of the edge.
func (daq *DrugAllergyQuery) WithManner(opts ...func(*MannerQuery)) *DrugAllergyQuery {
	query := &MannerQuery{config: daq.config}
	for _, opt := range opts {
		opt(query)
	}
	daq.withManner = query
	return daq
}
...
func (daq *DrugAllergyQuery) sqlAll(ctx context.Context) ([]*DrugAllergy, error) {
	...
	if daq.withDoctor != nil || daq.withPatient != nil || daq.withMedicine != nil || daq.withManner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := daq.withDoctor; query != nil {
	...
	if query := daq.withManner; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*DrugAllergy)
		for i := range nodes {
			if fk := nodes[i].manner_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(manner.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "manner_id" returned %v`, n.ID)
			}
			for i := range nodes {
