...
// The MannerFunc type is an adapter to allow the use of ordinary
// function as Manner mutator.
type MannerFunc func(context.Context, *ent.MannerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MannerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MannerMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MannerMutation", m)
	}
	return f(ctx, mv)
}
