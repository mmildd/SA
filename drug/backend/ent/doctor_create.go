...
func (dc *DoctorCreate) AddDoctorDrugAllergyIDs(ids ...int) *DoctorCreate {
	dc.mutation.AddDoctorDrugAllergyIDs(ids...)
	return dc
}

// AddDoctorDrugAllergy adds the Doctor_DrugAllergy edges to DrugAllergy.
func (dc *DoctorCreate) AddDoctorDrugAllergy(d ...*DrugAllergy) *DoctorCreate {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return dc.AddDoctorDrugAllergyIDs(ids...)
}
...
func (dc *DoctorCreate) createSpec() (*Doctor, *sqlgraph.CreateSpec) {
	...
	if nodes := dc.mutation.DoctorDrugAllergyIDs(); len(nodes) > 0 {
