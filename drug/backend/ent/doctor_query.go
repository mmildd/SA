...
type DoctorQuery struct {
	...
	// eager-loading edges.
	withDoctorDrugAllergy *DrugAllergyQuery
...
// QueryDoctorDrugAllergy chains the current query on the Doctor_DrugAllergy edge.
func (dq *DoctorQuery) QueryDoctorDrugAllergy() *DrugAllergyQuery {
	query := &DrugAllergyQuery{config: dq.config}
...
//  WithDoctorDrugAllergy tells the query-builder to eager-loads the nodes that are connected to
// the "Doctor_DrugAllergy" edge. The optional arguments used to configure the query builder of the edge.
func (dq *DoctorQuery) WithDoctorDrugAllergy(opts ...func(*DrugAllergyQuery)) *DoctorQuery {
	query := &DrugAllergyQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withDoctorDrugAllergy = query
	return dq
}
...
func (dq *DoctorQuery) sqlAll(ctx context.Context) ([]*Doctor, error) {
	...
	if query := dq.withDoctorDrugAllergy; query != nil {
		...
		query.Where(predicate.DrugAllergy(func(s *sql.Selector) {
			s.Where(sql.InValues(doctor.DoctorDrugAllergyColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.DoctorDrugAllergy = append(node.Edges.DoctorDrugAllergy, n)
		}
	}
