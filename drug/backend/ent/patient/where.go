...
// HasPatientDrugAllergy applies the HasEdge predicate on the "Patient_DrugAllergy" edge.
func HasPatientDrugAllergy() predicate.Patient {
	return predicate.Patient(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(PatientDrugAllergyTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, PatientDrugAllergyTable, PatientDrugAllergyColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasPatientDrugAllergyWith applies the HasEdge predicate on the "Patient_DrugAllergy" edge with a given conditions (other predicates).
func HasPatientDrugAllergyWith(preds ...predicate.DrugAllergy) predicate.Patient {
	return predicate.Patient(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(PatientDrugAllergyInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, PatientDrugAllergyTable, PatientDrugAllergyColumn),
		)
